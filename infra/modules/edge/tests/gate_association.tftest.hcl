...
# content-type is what lets API Gateway parse the body at all: without it the
# SQS integration's mapping template has nothing to forward and the join is
# accepted but empty.
#
...
run "the_join_policy_whitelists_content_type" {
  command = plan

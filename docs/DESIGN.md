  ...
  T−0   EventBridge Scheduler → seal_event (Rust, arm64)
          ONE UpdateItem on Counters:
            ...
            ConditionExpression: attribute_not_exists(shuffle_seed)
                                    │
                                    ▼
  T+    /queue_num reads i from PreQueue, seed and N from /status, returns
        PRP(seed, i, N) — a keyed permutation evaluated per read
```
...
live join (§6). The handler claims the next registration index from an atomic counter and
writes one `PreQueue` item:
...
index. Registration writes spread across the pre-queue window — minutes to hours — rather
than concentrating at T−0.
...
At T−0 the phase Lambda performs one `UpdateItem` on `Counters`:
...
`/queue_num` reads the visitor's `PreQueue` item for `i` and the seed and count from
`/status`. Both are already being fetched.
...
Given `shuffle_seed`, `participant_count`, and the `(request_id, i)` pairs in `PreQueue`, any
third party recomputes every position and confirms the ordering.
...
A sequence must yield a unique ordered value; summing shards cannot produce one. Write
sharding here is a throughput technique, distinct from shuffle sharding, which is an
isolation technique ([ADR-0008](adr/0008-partition-isolation-not-shuffle-sharding.md)).
...
**`PreQueue`** — partition key `r`. Attributes `i`, `t`. Short attribute names because the
table is scanned during audit. Read by `/queue_num` as a single `GetItem`; never scanned on
the hot path.

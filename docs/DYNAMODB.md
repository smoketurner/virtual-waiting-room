...
| Every debounced admin write | `attribute_not_exists(last_action_epoch_ms) OR last_action_epoch_ms <= :cutoff OR last_action_epoch_ms > :now` | A double-submitted form is a no-op for 2,000 ms |

Forcing maintenance mode is guarded on the expected phase but **not** debounced. An emergency stop
must always apply.

The debounce's last clause is what keeps a stamp later than `:now` from closing the control plane
permanently: such a stamp describes no prior action, and no operator action would move it back into
the past.

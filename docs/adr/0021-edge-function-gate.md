...
**Terraform generates it and writes both copies.** `random_bytes.signing_key` feeds the SSM
SecureString the Lambdas read and the KeyValueStore's `k` the gate reads, in one apply. One value
from one source means the two cannot disagree, and there is no separate step whose absence has to
be detected.

An earlier revision of this ADR instead seeded both with a placeholder literal under
`lifecycle { ignore_changes = [value] }` and had a script overwrite them out of band, to keep the
secret out of Terraform state. That reasoning does not survive contact with this repo: the RSA key
this one replaces was generated by `tls_private_key`, which put a private key in state, and the S3
backend encrypts it. The placeholder bought nothing and cost a whole class of failure — a
deployment where both stores agreed on a literal published in this repository, which no consistency
check could catch because the two copies genuinely matched. The OIDC client secret keeps the
placeholder pattern, because the identity provider issues it and Terraform cannot generate it.

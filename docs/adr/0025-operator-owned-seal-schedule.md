# ADR-0025: The operator owns the open schedule's time; Terraform owns the schedule

**Status:** Accepted

## 1. Context

The event's start time was `open_start_time`, a Terraform variable that decided whether an
...
Terraform creates `aws_scheduler_schedule.open` unconditionally, with its target, target input,
...
**Why not have the controller fire the open?** It already runs six passes a minute and the open is
...
silently drop the target's `input` — the event id the open reads — and its retry policy. The writer
...
regression assertable in a test. A bounded age is correct on its own terms too — an open delivered 24
...
how the system behaved before start times existed, and self-corrects at the open when the phase
...
`Counters` without a start time, and an armed schedule would open a cohort whose rows it just
deleted.
...
`lambda:InvokeFunction` on `open_event`. With no `CreateSchedule` or `DeleteSchedule` granted, the
...
- Clearing a start time disables the schedule rather than deleting it — enforced structurally: the
  `OpenSchedule` port has no delete operation and the IAM policy grants none.

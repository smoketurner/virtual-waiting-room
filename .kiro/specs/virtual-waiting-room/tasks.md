...
- [x] Signing-key generation and rotation: `scripts/bootstrap_edge_gate.py` generates the secret (or reuses an already-real one), writes SSM, then mirrors it to the edge gate's KeyValueStore in one run; refuses to overwrite a real secret without `--force`. `generate_token` and `authorizer` both refuse to start if the key they read is still the Terraform-seeded placeholder — the only guard against the bootstrap never having run (issue #71)
...
- [ ] Admin API: `/admin/phase`, `/admin/rate`, `/admin/message`, `/admin/reset`, `/admin/rules`, `/metrics`, `/update_session` [F3.10, F5.5] — Partial: phase, rate, message, reset, pause, resume, fail_open, and recover are live; `/admin/rules`, `/metrics`, and `/update_session` return the deferred stub. The edge gate's ruleset and `enforce_from` (issue #71) are written directly to the KeyValueStore today, not through an admin route.
